...
function SearchInput() {
    ...
    SpeechRecognition.startListening({language: 'VN'})
  ...
  return (
    <div ref={ref} className=' relative max-w-md sm:max-w-xl w-full flex items-center  transition duration-200   mx-auto group'>
        <div className={searchResults?.items?.length > 0 && showSuggest ? 'focus-within:shadow-md py-[10px] px-3 border rounded-3xl flex items-center w-full rounded-br-none rounded-bl-none' : 'focus-within:shadow-md py-[10px] px-3 border rounded-3xl flex items-center w-full'}>
            ...
            <input onFocus={() => onFocusSearchInput()} value={searchInput} onChange={(e) => onChangeSearch(e)} className='outline-none px-2 flex-grow' type="text" />

            <svg className='cursor-pointer w-6' focusable="false" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path fill="#4285f4" d="m12 15c1.66 0 3-1.31 3-2.97v-7.02c0-1.66-1.34-3.01-3-3.01s-3 1.34-3 3.01v7.02c0 1.66 1.34 2.97 3 2.97z"></path><path fill="#34a853" d="m11 18.08h2v3.92h-2z"></path><path fill="#fbbc05" d="m7.05 16.87c-1.27-1.33-2.05-2.83-2.05-4.87h2c0 1.45 0.56 2.42 1.47 3.38v0.32l-1.15 1.18z"></path><path fill="#ea4335" d="m12 16.93a4.97 5.25 0 0 1 -3.54 -1.55l-1.41 1.49c1.26 1.34 3.02 2.13 4.95 2.13 3.87 0 6.99-2.92 6.99-7h-1.99c0 2.92-2.24 4.93-5 4.93z"></path></svg>
        </div>
            ...
            <div className={showSuggest ? 'block' : 'hidden'}>
                <div className="group-focus-within:shadow-lg absolute top-full z-20 bg-white left-0 right-0  py-2 border rounded-br-3xl  rounded-bl-3xl border-t-0">
                    { searchResults?.items?.map((item: any, index: number) => {
                        return (
                            <div onClick={() => navigateToResults(item?.title)} key={index} className='grid grid-cols-[auto_1fr] items-center gap-3 cursor-pointer transition duration-200 hover:bg-gray-100  px-4 py-2'>
                                <SearchIcon className='h-4 text-gray-400 ' />
                                <p className='line-clamp-1' dangerouslySetInnerHTML={{__html: item.htmlTitle}}></p>
                            </div>
                        )
                    }) }
